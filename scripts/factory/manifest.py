...
KNOWN_FIELDS = {"file", "job_id", "credits_spent", "reject_reason", "notes",
                "attempts", "prompt_sent", "card_sha", "accepted_by"}
ALLOWED = {
    "pending": {"generating"},
    ...
    # `done` перестал быть тупиком: гейт устаревшего референса (card_sha) советует
    # `review.py requeue`, а совет был невыполним — принятый референс лежит именно
    # в `done`, и проект вставал намертво, разблокировать нечем, кроме правки
    # manifest.json руками. Терминальность защищала принятое от переработки, но
    # появилась раньше, чем правка карточки научилась делать принятое устаревшим.
    # Переход только по явной команде человека с конкретным id — это и есть тот
    # самый осознанный акт, ради которого защита ставилась.
    "done": {"pending"},
}
