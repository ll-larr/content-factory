...
SFX_VOLUME = 0.7
MUSIC_VOLUME = 0.35
# Приглушение музыки под репликами/SFX. Параметры выбраны на слух 2026-08-01 из
# семи вариантов (spike/duck_calib, файлы A-G + FINAL; суммарное приглушение
# относительно недакнутой подложки: A -2.5, B -3.2, C -1.1, G -4.3, E -5.0,
# F -6.1, выбранный -6.3 dB).
#   threshold=0.01 — низкий порог: дакинг срабатывает и на тихих репликах, а не
#     только на выкриках; на 0.03 (вариант B) музыка «приседала» слишком слабо.
#   ratio=20 — максимум фильтра: реплика должна доминировать, детский мульт
#     смотрят на плохой акустике, разборчивость важнее плотности подложки.
#   attack=5 — короткий: иначе музыка «наезжает» на первый слог фразы.
#   release=1050 — длинный: на 250-900 мс музыка возвращалась слишком быстро и
#     между короткими фразами слышно было «дыхание» громкости. При 1050 мс
#     подложка держится приглушённой всю реплику и поднимается после неё.
#     Значение выбрано пользователем на слух между 900 и 1150 мс.
# Вариант без дакинга отвергнут: реплики тонут в музыке.
DUCK = "sidechaincompress=threshold=0.01:ratio=20:attack=5:release=1050"
