...
Для ревьюера — человека и Claude-режиссёра. Запускать из корня репозитория:
  ...
  python scripts/review.py --project projects/<проект> accept <id> [<id> ...]
  python scripts/review.py --project projects/<проект> accept-notes <id> --notes "..."
  python scripts/review.py --project projects/<проект> reject <id> --reason "..."
  python scripts/review.py --project projects/<проект> requeue <id>

requeue — решение человека перегенерировать: возвращает rejected,
accepted_with_notes или done в pending; reject_count при этом НЕ сбрасывается
(журнал). Принятое (done) возвращают, когда его основание изменилось — например
правка карточки персонажа сделала принятый референс устаревшим, и гейт
раскадровки требует новый.
"""
...
def main(argv=None) -> int:
    ...
    p_requeue = sub.add_parser(
        "requeue", help="rejected | accepted_with_notes | done -> pending")
    ...
    if args.command == "list":
        ...
        for item_id in sorted(manifest.data["items"]):
            ...
            shown += 1
        if not shown:
            # Пустой вывод неотличим от неработающей команды: на первом живом
            # прогоне `list --status generated` промолчал, и понять, «нечего
            # ревьюить» это или «команда сломалась», было нельзя.
            where = f" со статусом {args.status}" if args.status else ""
            print(f"нечего показать: в манифесте нет ни одного item{where}")
        return 0
