...
def test_done_reopens_only_into_pending(tmp_path):
    """`done` — не тупик, но и не проходной двор.

    Единственный выход из принятого — обратно в очередь на перегенерацию: этого
    требует гейт устаревшего референса, который советует `review.py requeue`
    (раньше совет был невыполним, см. ALLOWED). Прямые переходы в `generated`,
    `rejected` и `accepted_with_notes` по-прежнему запрещены: заново принять или
    отклонить можно только то, что заново сгенерировано.
    """
    m = Manifest(tmp_path / "manifest.json")
    m.add("x", kind="frame")
    ...
    m.set_status("x", "done")
    for forbidden in ("generated", "rejected", "accepted_with_notes"):
        with pytest.raises(ManifestError, match="not allowed"):
            m.set_status("x", forbidden)
...
def test_generation_ends_in_generated_then_review_accepts(tmp_path):
    ...
    assert m.get("x")["status"] == "done"
    # Что можно и чего нельзя после `done` — в test_done_reopens_only_into_pending.
