...
def test_list_on_missing_manifest_says_nothing_to_show(tmp_path, capsys):
    """Раньше печаталась пустота, неотличимая от неработающей команды."""
    pdir = tmp_path / "projects" / "empty"
    pdir.mkdir(parents=True)
    assert run(pdir, "list") == 0
    assert "нечего показать" in capsys.readouterr().out
...
def test_list_says_so_when_nothing_matches(proj, capsys):
    """Пустой вывод неотличим от неработающей команды — на первом живом прогоне
    `list --status generated` просто промолчал."""
    assert run(proj, "list", "--status", "rejected") == 0
    out = capsys.readouterr().out
    assert "нечего показать" in out and "rejected" in out
