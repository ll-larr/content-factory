...
import pytest
from factory.models import (MODEL_DIRS, ModelError, estimate_media_cost,
                            find_card, load_card, validate_video_model)
...
def test_all_real_knowledge_cards_load():
    ...
    # Карточки моделей лежат в MODEL_DIRS; верхний уровень — _template.md и
    # *-api.md (контракты), а knowledge/genres/ — карточки жанров со своей
    # схемой, их проверяет tests/test_genres.py.
    cards = [p for d in MODEL_DIRS
             for p in sorted((Path("knowledge") / d).glob("*.md"))
...
def test_unclosed_frontmatter_raises_model_error(tmp_path):
    ...
    with pytest.raises(ModelError, match="не закрыт"):
        load_card(kdir / "video" / "bad.md")


def test_empty_frontmatter_raises_model_error(tmp_path):
    """Пустой frontmatter парсится split_frontmatter в {} (валидный YAML-словарь),
    поэтому спотыкается не о 'не словарь', а о проверку обязательных полей карточки —
    сообщение точнее указывает причину."""
    kdir = write_card(tmp_path, "---\n---\nbody\n", name="bad.md")
...
def test_invalid_yaml_raises_model_error(tmp_path):
    kdir = write_card(tmp_path, "---\nkey: [unclosed\n---\nbody\n", name="bad.md")
    with pytest.raises(ModelError, match="некорректный YAML"):
        load_card(kdir / "video" / "bad.md")
...
# --- статус catalog: трата разрешена без живой пробы (решение 2026-09-04) ---
...
providers:
  wavespeed:
    ...
    pricing: flat
    tiers:
      lite: { id: "g/veo-lite", usd_per_image: 0.30 }
    default_tier: lite
---
# veo probe
"""
...
def test_catalog_status_does_not_block_spending():
    # skeleton блокирует, catalog — нет: человек решил тратить, зная, что цена
    # взята из каталога провайдера, а не подтверждена живой генерацией.
    card = parse_card(CATALOG_VIDEO)
    assert validate_video_model(card, 5, "wavespeed") == []


def test_catalog_status_still_checks_provider():
    card = parse_card(CATALOG_VIDEO)
    problems = validate_video_model(card, 5, "runware")
    assert any("not available on provider" in p for p in problems)


def test_skeleton_still_blocks():
    card = parse_card(CATALOG_VIDEO.replace("status: catalog", "status: skeleton"))
    assert any("skeleton" in p for p in validate_video_model(card, 5, "wavespeed"))


def test_unknown_status_is_reported():
    # Опечатка в статусе не должна молча открывать трату.
    card = parse_card(CATALOG_VIDEO.replace("status: catalog", "status: verifed"))
    assert any("verifed" in p for p in validate_video_model(card, 5, "wavespeed"))


# --- валидатор аудио-модели: провайдер и роль (дизайн 2026-09-04 §4) ---

from factory.models import validate_audio_model  # noqa: E402
...
def test_audio_model_ok_on_declared_provider():
    assert validate_audio_model(parse_card(AUDIO_TTS), "wavespeed", "tts") == []


def test_audio_model_rejected_on_other_provider():
    # Карточка, открытая живой пробой у одного провайдера, не должна открывать
    # трату у другого, где id и цена не подтверждены.
    problems = validate_audio_model(parse_card(AUDIO_TTS), "runware", "tts")
    assert any("not available on provider" in p for p in problems)


def test_audio_model_rejected_in_wrong_role():
    # Музыкальная модель в роли озвучки вскрылась бы только на слух.
    problems = validate_audio_model(parse_card(AUDIO_TTS), "wavespeed", "music")
    assert any("audio_kind" in p for p in problems)


def test_audio_model_rejects_non_audio_card():
    card = parse_card(AUDIO_TTS.replace("type: audio", "type: image"))
    assert any("not an audio model" in p for p in validate_audio_model(card))


def test_audio_skeleton_blocks():
    card = parse_card(AUDIO_TTS.replace("status: verified", "status: skeleton"))
    assert any("skeleton" in p for p in validate_audio_model(card, "wavespeed", "tts"))
