...
@pytest.mark.parametrize("name", list(PROVIDERS))
def test_preflight_reports_missing_api_key(name, kdir, monkeypatch):
    """D-2: отсутствующий ключ — ошибка конфигурации, известная до сети. Ловит её
    слой провайдера (изоляция провайдера — правило репо), по объявленному
    api_key_env, тем же хуком preflight_problems, который generate_batch зовёт ДО
    сметы. Раньше это всплывало в submit по строке на каждую единицу батча."""
    monkeypatch.delenv(PROVIDERS[name]["env"], raising=False)
    p = make(name, kdir)
    problems = p.preflight_problems("seedance_2_0", {"resolution": "720p",
                                                     "duration": 5})
    assert any(PROVIDERS[name]["env"] in x for x in problems), problems


@pytest.mark.parametrize("name", list(PROVIDERS))
def test_preflight_silent_when_api_key_present(name, kdir, monkeypatch):
    monkeypatch.setenv(PROVIDERS[name]["env"], "test-key")
    p = make(name, kdir)
    assert p.preflight_problems("seedance_2_0", {"resolution": "720p",
                                                 "duration": 5}) == []


def test_preflight_needs_no_key_when_api_key_env_not_declared(kdir, monkeypatch):
    """Пустое api_key_env означает «ключ не нужен», а не «ключ под пустым именем».
    На этом держатся фейковые провайдеры тестов: объявить проверку ключа в базе и
    заодно потребовать переменную от того, кто её не объявлял, — регрессия."""
    monkeypatch.delenv("", raising=False)

    class Keyless(BaseHTTPProvider):
        name = "keyless"

    assert Keyless(knowledge_dir=kdir).preflight_problems("seedance_2_0", {}) == []


def test_runware_preflight_problems_unknown_resolution(kdir, monkeypatch):
    ...
    отдельно — здесь. Ключ в окружении задан: этот тест про разрешение, а
    отсутствие ключа — отдельная проблема того же хука (тесты выше)."""
    monkeypatch.setenv("RUNWARE_API_KEY", "test-key")
    p = make("runware", kdir)
    problems = p.preflight_problems("seedance_2_0", {"resolution": "1080p"})
    assert problems
    assert any("1080p" in x for x in problems), problems


def test_runware_preflight_problems_known_resolution_ok(kdir, monkeypatch):
    monkeypatch.setenv("RUNWARE_API_KEY", "test-key")
    p = make("runware", kdir)
    assert p.preflight_problems("seedance_2_0", {"resolution": "720p"}) == []


def test_runware_preflight_problems_no_resolution_ok(kdir, monkeypatch):
...
status: verified
providers:
  wavespeed: { id: "vendor/tts-model", usd_per_image: 0.02, fields: { text: text, language: language, voice: voice } }
---
# TTS probe
"""
...
status: verified
providers:
  wavespeed: { id: "vendor/foley-model", usd_per_image: 0.001, fields: { prompt: prompt, video: video, duration: duration } }
---
# Foley probe
"""
...
status: verified
providers:
  wavespeed: { id: "v/m", usd_per_image: 0.01 }
---
# bad
"""
...
@pytest.fixture
def audio_kdir(tmp_path):
    d = tmp_path / "knowledge" / "audio"
    d.mkdir(parents=True)
    (d / "tts_probe.md").write_text(AUDIO_CARD, encoding="utf-8")
    (d / "foley_probe.md").write_text(FOLEY_CARD, encoding="utf-8")
    (d / "bad_audio.md").write_text(BAD_AUDIO_CARD, encoding="utf-8")
    return tmp_path / "knowledge"


def test_wavespeed_audio_maps_fields_from_card(audio_kdir, monkeypatch):
    captured = {}
    ...
    p = WaveSpeedProvider(audio_kdir)
    monkeypatch.setenv("WAVESPEED_API_KEY", "k")
    monkeypatch.setattr(p, "_request", fake_request)
    job = p.submit("tts_probe", {"text": "hi", "language": "Russian (Russia)",
                                 "voice": "Kore"})
    assert job == "task-a"
    assert captured["url"] == "https://api.wavespeed.ai/api/v3/vendor/tts-model"
    assert captured["body"] == {"text": "hi", "language": "Russian (Russia)",
                                "voice": "Kore"}


def test_wavespeed_audio_omits_absent_params(audio_kdir, monkeypatch):
    # Ключ, которого нет в params, не уезжает пустым: у части моделей пустая
    # строка — валидное значение и означает не то же, что отсутствие ключа.
    captured = {}
    ...
    monkeypatch.setenv("WAVESPEED_API_KEY", "k")
    monkeypatch.setattr(p, "_request", fake_request)
    p.submit("tts_probe", {"text": "hi"})
    assert captured["body"] == {"text": "hi"}


def test_wavespeed_audio_media_param_becomes_data_uri(audio_kdir, monkeypatch, tmp_path):
    # video/audio-входы — такие же медиа, как кадры: локальный путь кодируется в
    # data-URI, готовый URL пропускается как есть.
    seg = tmp_path / "001.mp4"
    seg.write_bytes(b"ab")
    captured = {}
    ...
    monkeypatch.setenv("WAVESPEED_API_KEY", "k")
    monkeypatch.setattr(p, "_request", fake_request)
    p.submit("foley_probe", {"prompt": "boots", "video": str(seg), "duration": 5})
    assert captured["body"]["video"].startswith("data:video/mp4;base64,")
    assert captured["body"]["prompt"] == "boots"
    assert captured["body"]["duration"] == 5


def test_wavespeed_audio_card_without_fields_raises(audio_kdir, monkeypatch):
    # Карточка без карты полей — ошибка контракта, а не повод угадывать имена.
    p = WaveSpeedProvider(audio_kdir)
    monkeypatch.setenv("WAVESPEED_API_KEY", "k")
    with pytest.raises(ProviderError, match="fields"):
        p.submit("bad_audio", {"text": "x"})


# --- баланс: панель показывает деньги там, где провайдер их отдаёт ---

def test_only_wavespeed_supports_balance(kdir):
    """Выдуманный баланс хуже отсутствующего: по нему решают, тратить ли."""
    assert WaveSpeedProvider(kdir).supports_balance is True
    assert RunwareProvider(kdir).supports_balance is False
    assert OpenRouterProvider(kdir).supports_balance is False


def test_provider_without_support_raises_on_balance(kdir):
    with pytest.raises(ProviderError):
        RunwareProvider(kdir).balance()


def test_wavespeed_balance_hits_documented_endpoint(kdir, monkeypatch):
    seen = {}

    def fake_request(method, url, json_body=None):
        seen["method"], seen["url"] = method, url
        return {"code": 200, "data": {"balance": 41.07}}

    p = WaveSpeedProvider(kdir)
    monkeypatch.setattr(p, "_request", fake_request)

    assert p.balance() == 41.07
    assert seen["method"] == "GET"
    assert seen["url"].endswith("/balance")


def test_wavespeed_balance_reads_flat_response(kdir, monkeypatch):
    """Схема ответа живьём снята один раз; форму на верхнем уровне тоже терпим."""
    p = WaveSpeedProvider(kdir)
    monkeypatch.setattr(p, "_request", lambda *a, **k: {"balance": "3.5"})
    assert p.balance() == 3.5


def test_wavespeed_balance_without_number_raises(kdir, monkeypatch):
    p = WaveSpeedProvider(kdir)
    monkeypatch.setattr(p, "_request", lambda *a, **k: {"data": {}})
    with pytest.raises(ProviderError):
        p.balance()
