...
- **Генерация звука не входит в конвейер.** Стадия `audio` удалена: TTS через Higgsfield
  не заработал, и озвучка вынесена в отдельную задачу на ElevenLabs. `mix_audio` и `assemble`
  сохранены и ждут готовые аудиофайлы, прописанные в манифесте.
- **В `master` все карточки кадров и отрезков имеют `status: skeleton`**, то есть генерация
  заблокирована гейтом. Живьём подтверждённые маппинги (8 карточек: z_image_turbo,
  flux_2_klein, seedream_v4_5, nano_banana_flash, nano_banana_2, veo3_1_lite, vidu_q2_turbo,
  kling3_0) лежат в ветке `spike/provider-verification` и ещё не слиты.
- **Runware не проверен живьём** — контракт запросов верный, но спайк упёрся в нулевой баланс.
- Первый этап конвейера — от брифа к `shots.json` — пока делается вручную, автоматизация
  запланирована.
